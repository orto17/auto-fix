...
setAutoFixVersion() {
  ...
  then
      VERSION=$1
      echo "Downloading version $VERSION of JFrog Auto-Fix..."
  else
      echo "Downloading the latest version of JFrog Auto-Fix..."
  fi
}

setAutoFixRemoteRepositoryIfNeeded() {
...
setWindowsProperties() {
  AUTO_FIX_OS="windows"
  URL="${PLATFORM_URL}/artifactory/${REMOTE_PATH}jfrog-auto-fix/v1/${VERSION}/jfrog-auto-fix-windows-amd64/jfrog-auto-fix.exe"
  FILE_NAME="jfrog-auto-fix.exe"
}
...
setMacProperties() {
  ...
  URL="${PLATFORM_URL}/artifactory/${REMOTE_PATH}jfrog-auto-fix/v1/${VERSION}/jfrog-auto-fix-${AUTO_FIX_OS}-${ARCH}/jfrog-auto-fix"
  FILE_NAME="jfrog-auto-fix"
}
...
echoGreetings() {
  echo "JFrog Auto-Fix downloaded successfully!"
}
...
verify_download_or_exit() {
  if [ "${AUTO_FIX_INSECURE_SKIP_CHECKSUM_VERIFICATION:-}" = "1" ]; then
    echo "WARNING: skipping checksum verification (AUTO_FIX_INSECURE_SKIP_CHECKSUM_VERIFICATION=1)." >&2
    echo "Downloaded ${FILE_NAME} (checksum verification skipped)."
    return 0
  fi

  headers=$(head_request "${URL}") || {
    echo "Failed to fetch Artifactory file details for this Auto-Fix build." >&2
    rm -f "${FILE_NAME}"
    exit 1
  }
